...
$("#multiplefileupload").fileinput({
    ...
    dropZoneTitle:
        '<div class="upload-area"><i class="far fa-images"></i><p>আপলোড সাইন কপি</p> <div> <button type="button">আপলোড </button> </div></div>',
    dropZoneClickTitle: "",
...
async function uploadFilePdfToText(myfile) {
  ...
  /*  try {
        const res = await fetch("https://api.priyoshop.xyz/pdftoText", {
            method: "post",
            body: formData,
        ...
        $("#loadMe").modal("hide");
        return swal("Something want wrong! try again..");
    }
*/    
}
...
function downloadPdfSave(blance,cost){
    if(blance < cost){
        swal("Influence balance please add money");
        $("#myModal").modal('hide');
    }else{
        $("#myModal").modal('hide');
...
$("#submitalldatafornid").on("submit", async function (e) {
    ...
    try {
        ...
        const res1 = await fetch("https://api.priyoshop.xyz/makeNid", {
            method: "POST",
            body: jsonData,
            headers: {
